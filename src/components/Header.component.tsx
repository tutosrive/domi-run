...
function HeaderComponent() {
  ...
  const user_avatar = () => {
    ...
    return <>{user.id ? <Avatar image={''} label={user.givenName?.slice(0, 1)} shape="circle" /> : '-'}</>;
  };
    ...
    <>
      ...
      <div className="navbar bg-base-100 shadow-sm">
        ...
        <div className="flex-1">
          {/* Open Sidebar */}
          <Button icon="pi pi-bars" style={{ backgroundColor: 'transparent', color: 'white', border: 'none', height: '40px' }} onClick={() => dispatchEvent(setOpen())} />
          <Link className="btn btn-ghost text-xl" to={'/'}>
            Domi Run
          </Link>
        </div>
        <div className="flex-none">
          {/* DropDown "Shoppign car" */}
          ...
          {user.id ? (
            ...
            //   Button to SigIn (Without dropdown)
            <SigInButtonComponent classes={'mx-2'} icon={'pi pi-sign-in'} styles={{ color: 'white', border: 'none', backgroundColor: 'transparent' }} />
          )}
        </div>
      </div>
    </>
